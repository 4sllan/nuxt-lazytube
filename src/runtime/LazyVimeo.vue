...
import {calcAspect, createIframe, fetchingOembed, getVimeoID, isPostMessageSupported} from "./utils";
...
const processedThumbnail = computed(() => {
  return props.customThumbnail ? props.customThumbnail : (videoInfo.value !== null) ? videoInfo.value['thumbnail_url'] : ''
})
...
const getFetchingOembed = () => {
  if (props.oembedFetch) {
    // Fetch Oembed
    fetchingOembed(props.src, 'vimeo')
